...
type RestOf<T extends ITask | NewTask> = Omit<T, 'deadline' | 'createdDate'> & {
...
export class TaskService {
  ...
  protected convertDateFromClient<T extends ITask | NewTask | PartialUpdateTask>(task: T): RestOf<T> {
    return {
      ...task,
      deadline: task.deadline?.toJSON() ?? null,
      createdDate: task.createdDate?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restTask: RestTask): ITask {
    return {
      ...restTask,
      deadline: restTask.deadline ? dayjs(restTask.deadline) : undefined,
